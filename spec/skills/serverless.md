...
- **In verify (proof):** the control holds only when each function runs least-privileged and self-contained, by the Safer shapes of its block. Judge one function at a time, since the role, trigger, and egress are per-function, so a hardened neighbour proves nothing about this one. If a single compromised or coerced function can still reach beyond its scope, act on unvalidated event data, leak across invocations, drive the chain, pivot the network, or expose a secret, the risk is not closed: record it and point back to harden.
