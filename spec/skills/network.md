...
The hook exits non-zero when any destination reads `private-target` or `parser-divergent`, and exits zero when every line is `safe` or `invalid url`.
...
- **In verify (proof):** the control holds only when the destination is no longer assembled from input, by the Safer shapes of its block. The checker is deterministic, so when you run it on a destination the code actually builds and it reads `private-target` or `parser-divergent`, that is the risk open, full stop. The only verify case where such a verdict is benign is when you deliberately feed it a literal the validation must reject, to confirm the control rejects it: there the verdict describes your test input, and closure is proven by reading the code and seeing the validation actually reject that form. The checker does not prove the runtime parts (DNS rebinding, redirects followed at runtime, egress), which still need their own reasoning. If a destination the code builds can reach an internal target, or hand the user or their form to an attacker-chosen origin by any path, the risk is not closed: record it and point back to harden.
