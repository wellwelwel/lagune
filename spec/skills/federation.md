...
- **In verify (proof):** the control holds only when a forged, swapped, or replayed proof is demonstrably rejected, never merely when a genuine login works. Confirm each refusal: a token with `alg: none`, a public-key-as-HMAC `RS256`-to-`HS256` swap, an edited payload, a wrong `aud` or expired `exp`, an OAuth callback with a missing or mismatched `state` or an unregistered `redirect_uri`, a SAML response with an unsigned, wrapped, or wrong-audience assertion, and a relayed QR or unbound cross-device approval. If any forgeable or replayable proof can still mint a session, the risk is not closed: record it as such and point back to harden.
