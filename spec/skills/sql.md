...
- **In verify (proof):** the control holds only when untrusted input can no longer change what the engine parses, **and** when the account behind the connection cannot exceed what the application needs. Proving one axis is not closure: flawless binding under an owner or administrator account is still open, and a least-privileged account under one concatenated identifier is still open. Prove the exposure and limit controls by observation: an error returned to the caller reveals no schema, a listing endpoint refuses to return everything at once, and a query that runs too long is stopped by the engine. If a crafted value can still change the shape of a statement, or a compromised connection still reaches beyond the application's data, the risk is not closed: record it as such and point back to harden.
