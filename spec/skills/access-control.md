...
- **In verify (proof):** prove each link against the Safer shape of its block, never by checking the UI hides the path. The closed conditions: another user's object id is refused, a privileged endpoint is denied to an unprivileged caller on every HTTP method, a tampered price, role, or `tenant_id` is rejected, an over-posted field never reaches the model, a cross-tenant query returns nothing, a sensitive action demands step-up and a replayed approval fails, recovery and signup answer identically whether or not the account exists, and the session id regenerates at login and dies at logout. If any client-controlled value can still select an object, drive a decision, cross a tenant boundary, or stand in for identity, the risk is not closed: record it as such and point back to harden.
