...
- **In verify (proof):** the control holds only when untrusted input no longer reaches the interpreter as instruction, and the blocks close in three ways. Evaluation, include, and dispatch close on the **value**, reduced to a validated key into a code-defined set, or the evaluator removed. The general injection case closes on the **interface**, a parameterized or safe API carrying the data, never escaping alone. Loaded code closes on the **origin**, since no user value is involved. If a crafted value can still change what the interpreter does, or code from an unverified origin can still run, the risk is not closed: record it as such and point back to harden.
