...
Does not close it: a secret in another cookie (every cookie auto-sends, so it forges too), accepting only `POST` (a cross-site form sends `POST`), multi-step flows (each step is forgeable), HTTPS alone, or a `Referer` check alone (stripped or spoofed too often to rely on). CORS is not a CSRF defense either: it governs whether the attacker can _read the response_, never whether the forged request _fires_. And no CSRF token survives XSS, which reads the token and forges with it, so XSS prevention is a prerequisite, not a substitute.
...
- **In verify (proof):** the control holds only when the request's provenance is established from something the caller cannot forge, by the Safer shapes of its block. The first three risks are one error on three different fields, so closing one proves nothing about the others: check the token, the origin, and the client IP separately. If any state change, cross-origin read, or IP-gated control can still be driven by a value the request supplies, or the server still volunteers its version, the risk is not closed: record it and point back to harden.
