...
It reads the material and distills it into a focused, defense-only sub-skill, shaped exactly like the built-in ones, so [`/bluespec.skills`](./skills.mdx) loads it afterwards like any other. It only audits and explains: it never writes attack inputs or exploits, and it never touches your code.
...
- The sub-skill at `.bluespec/skills/<name>.md`, following the [sub-skill shape](../skills.mdx).
