...
The `url-safety` hook decides whether a server-side fetch destination is safe from **SSRF** (a request that can be aimed at internal services, cloud metadata, or private hosts). It is the deterministic engine the [`ssrf` sub-skill](/docs/commands/skills) uses, and you can run it yourself.
...
This is the same check the [`ssrf` sub-skill](/docs/commands/skills) runs to prove a destination filter holds. Point it at each host in an allowlist or denylist, one call per host.
:::
